...
@section('title', 'Cadastrar Horário')

@section('content')
<div class="admin-page">
    <h1 class="mb-4">Cadastrar Horário</h1>
    ...
    <form action="{{ route('admin.horarios.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="barbeiro_id" class="form-select" required>
                ...
                @foreach($barbeiros as $barbeiro)
                    <option value="{{ $barbeiro->id }}" {{ old('barbeiro_id') == $barbeiro->id ? 'selected' : '' }}>{{ $barbeiro->nome }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-check mt-3">
            <input class="form-check-input" type="checkbox" name="disponivel" value="1" id="disponivel" {{ old('disponivel', true) ? 'checked' : '' }}>
